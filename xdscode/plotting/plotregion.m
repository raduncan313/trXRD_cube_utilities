function patchgrp=plotregion(A,b,lb,ub,c,transp,points,linetyp,start_end)
% The program plots closed region(s) in 2D/3D. The region(s) x is
% a subset of R2 or R3 such that Ax>=b and lb<=x<=ub.
...
if iscell(A)
    ...
    if n==2
        ...
        for pp=1:p
            ...
            ptch  = patch(X{pp}(1,ind3+1)',X{pp}(2,ind3+1)',c{pp});
            patchgrp = [patchgrp, ptch];
            set(ptch,'FaceAlpha',transp);
        end
    ...
    elseif n==3
        ...
        elseif nargin==9
            ...
            elseif length(start_end)==4
                ...
                plot3(points(1,end)',points(2,end)',points(3,end)',start_end(3:4));
                hold on;  
            end
        end
        ...
        for pp=1:p
            ...
            for i=1:m
                ...
                if lind2>0   
                    ...
                    [sooo,ind3]=sort(costhe);    
                    set(patch(X{pp}(1,ind2(ind3))',...
                        X{pp}(2,ind2(ind3))',...
                        X{pp}(3,ind2(ind3))',c{pp}),...
                        'FaceAlpha',transp);        
                end    
            end
    ...
    elseif n==3  
        ...
        for i=1:(m-2)
            for j=(i+1):(m-1)
                for k=(j+1):m
                    try
                        x=A([i j k],:)\b([i j k]);
                        if and(min((A*x-b))>-1e-12,min((A*x-b))<Inf)
                            X=[X,x];
                            eq=[eq,[i j k]'];
                        end
        ...
        if nargin==7
            ...
            hold on;
        elseif nargin==8
            plot3(points(1,:)',points(2,:)',points(3,:)',linetyp);
            hold on;            
        elseif nargin==9
            plot3(points(1,:)',points(2,:)',points(3,:)',linetyp);
            hold on;    
            if length(start_end)==2 
                plot3(points(1,1)',points(2,1)',points(3,1)',start_end);
                hold on;
                plot3(points(1,end)',points(2,end)',points(3,end)',start_end);
                hold on;
            elseif length(start_end)==4
                plot3(points(1,1)',points(2,1)',points(3,1)',start_end(1:2));
                hold on;
                plot3(points(1,end)',points(2,end)',points(3,end)',start_end(3:4));
                hold on;  
            end
        ...
        for i=1:m
            % each i, j or k in cols of eq represents a polygon face of the
            % boundary. The loop traverses all faces. Some appear collinear,
            % not triangles.
            % X(:,ind2) contains all the vertices that are part of face `i`
            [ind1,ind2]=find(eq==i);    
            lind2=length(ind2);  
            if lind2>0   
                xm=mean(X(:,ind2),2);         
                ...
                [sooo,ind3]=sort(costhe);   
                ptch=patch(X(1,ind2(ind3))',X(2,ind2(ind3))',X(3,ind2(ind3))',c);
                patchgrp = [patchgrp, ptch];
                set(ptch,'FaceAlpha',transp);        
            end    
        end   
