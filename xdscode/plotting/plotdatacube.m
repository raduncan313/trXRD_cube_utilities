function plotdatacube(varargin)
...
% imobj = plotcubeframe(imobj, geometry, data(:,:,1));
...
function means=selectroi(data)
    means = zeros(size(data,3),1);
    bw = roipoly;
    for ii=1:size(data,3)
        means(ii) = sum(sum(bw.*squeeze(data(:,:,ii))));
    end
end
