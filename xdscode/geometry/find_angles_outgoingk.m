function [phi, theta, chi, del, mu] = find_angles_outgoingk(geometry, hkl, sample_normal, theta, chi, x0)
...
gam1 = acosd(sample_normal(:)'*Ghkl./norm(Ghkl));
...
%polar coordinates
del = atan2d(kp(2), kp(1));

% mu = atand(kp1(2)/kp1(1));
mu = 90.0 - acosd(kp(3)*lambda0);
