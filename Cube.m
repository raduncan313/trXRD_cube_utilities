    ...
    methods
        ...
        function [inds_clsts, sigs_clsts] = auto_signal(obj, numcomponents, epsilon, minpts, frac)
            ...
            for ii = 1:length(idx_unq)
                ...
                sig_clst = mean(on_flat(jj1D,:), 1)./mean(off_flat(jj1D,:), 'all') - 1;
                sig_clst  = sig_clst/(max(sig_clst) - min(sig_clst));
                sig_clst = sig_clst + 1;
        ...
        function f = auto_signal_figure(obj, numcomponents, epsilon, minpts, frac)
            [inds_clsts, sigs_clsts] = obj.auto_signal(numcomponents, epsilon, minpts, frac);
            f = figure;
            ax = subplot(1,2,1);
        ...
        function f = auto_signal_GUI(obj, params)
            defparams = params;
            f = figure('position', [100 100 1000 500]);
            update_figure(f, params)
            ...
            panel_pca = uipanel(f, 'Position', [0.5 0.7 0.45 0.25],...
                'Title', 'PCA')

            nc_max = 20;
            nc_min = 2;
            sld_nc = uicontrol(panel_pca,...
            'units', 'normalized',...
            'position', [sld_x 0.5 sld_w sld_h],...
            'style', 'slider',...
            'min', nc_min,...
            'max', nc_max,...
            'value', params.nc,...
            'SliderStep', [1/(nc_max - nc_min), 5/(nc_max - nc_min)],...
            'callback', @callback);

            txt_nc = uicontrol(panel_pca,...
            'units', 'normalized',...
            'position', [txt_x 0.5 txt_w txt_h],...
            'style', 'text',...
            'string', sprintf('Components: %1$d', params.nc),...
            'fontsize', 12);

            panel_ocsvm = uipanel(f, 'Position', [0.5 0.4 0.45 0.25],...
                'Title', 'OCSVM')

            sld_frac = uicontrol(panel_ocsvm,...
            'units', 'normalized',...
            'position', [sld_x 0.5 sld_w sld_h],...
            'style', 'slider',...
            'min', 0.01,...
            ...
            txt_frac = uicontrol(panel_ocsvm,...
            'units', 'normalized',...
            'position', [txt_x 0.5 txt_w txt_h],...
            'style', 'text',...
            'string', sprintf('Fraction: %1$d', params.frac),...
            'fontsize', 12);

            panel_dbscan = uipanel(f, 'Position', [0.5 0.1 0.45 0.25],...
                'Title', 'DBSCAN')

            minpts_max = 20;
            minpts_min = 1;
            sld_minpts = uicontrol(panel_dbscan,...
            'units', 'normalized',...
            'position', [sld_x 0.67 sld_w sld_h],...
            'style', 'slider',...
            'min', minpts_min,...
            'max', minpts_max,...
            'SliderStep', [1/(minpts_max - minpts_min), 5/(minpts_max - minpts_min)],...
            'value', params.minpts,...
            'callback', @callback);

            txt_minpts = uicontrol(panel_dbscan,...
            'units', 'normalized',...
            'position', [txt_x 0.67 txt_w txt_h],...
            'style', 'text',...
            'string', sprintf('Min. points: %1$d', params.minpts),...
            'fontsize', 12);

            sld_eps = uicontrol(panel_dbscan,...
            'units', 'normalized',...
            'position', [sld_x 0.33 sld_w sld_h],...
            'style', 'slider',...
            'min', 1,...
            'max', 20,...
            'value', params.eps,...
            ...
            txt_eps = uicontrol(panel_dbscan,...
            'units', 'normalized',...
            'position', [txt_x 0.33 txt_w txt_h],...
            'style', 'text',...
            'string', sprintf('Epsilon: %1$d', params.eps),...
            'fontsize', 12);

            reset_button = uicontrol(f,...
                'Units', 'normalized',...
            ...
            function callback(~,~)
                sld_nc.Value = round(sld_nc.Value);
                sld_minpts.Value = round(sld_minpts.Value);

                params.nc = sld_nc.Value;
                params.frac = sld_frac.Value;
                params.minpts = sld_minpts.Value;
                params.eps = sld_eps.Value;
        
                txt_nc.String = sprintf('Components: %1$d', params.nc);
                txt_frac.String = sprintf('Fraction: %1$d', params.frac);
                txt_minpts.String = sprintf('Min. points: %1$d', params.minpts);
                txt_eps.String = sprintf('Epsilon: %1$d', params.eps);
            ...
            function reset(~,~)
                params = defparams;
                sld_nc.Value = params.nc;
                sld_frac.Value = params.frac;
                sld_minpts.Value = params.minpts;
                sld_eps.Value = params.eps;
            ...
            function update_figure(f, params)
                [inds_clsts, sigs_clsts] = obj.auto_signal(params.nc,...
                    params.eps, params.minpts, params.frac);
